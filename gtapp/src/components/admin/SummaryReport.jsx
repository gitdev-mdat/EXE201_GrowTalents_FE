...
const SummaryReport = () => {
  ...
  const summaryStats = {
    ...
    avgAttendance: Math.round(attendanceData.reduce((sum, item) => sum + item.rate, 0) / attendanceData.length),
    totalSalary: teacherSalaryData.reduce((sum, item) => sum + item.amount + item.bonus, 0),
  };
  ...
  const renderStatCards = () => (
    <Grid container spacing={3} className={styles.statsContainer}>
      <Grid item xs={12} sm={6} md={3}>
        <Card className={styles.statCard} style={{ borderLeft: '4px solid #667eea' }}>
          <CardContent className={styles.statContent}>
            <Box display="flex" alignItems="center" justifyContent="space-between">
              ...
              <AttachMoney className={styles.statIcon} style={{ color: '#667eea' }} />
            </Box>
          </CardContent>
        </Card>
      </Grid>
      ...
      <Grid item xs={12} sm={6} md={3}>
        <Card className={styles.statCard} style={{ borderLeft: '4px solid #f093fb' }}>
          <CardContent className={styles.statContent}>
            <Box display="flex" alignItems="center" justifyContent="space-between">
              <Box>
                <Typography variant="h6" className={styles.statTitle}>
              ...
              <PeopleAlt className={styles.statIcon} style={{ color: '#f093fb' }} />
            </Box>
          </CardContent>
        </Card>
      </Grid>
      ...
      <Grid item xs={12} sm={6} md={3}>
        <Card className={styles.statCard} style={{ borderLeft: '4px solid #4facfe' }}>
          <CardContent className={styles.statContent}>
            <Box display="flex" alignItems="center" justifyContent="space-between">
              <Box>
                <Typography variant="h6" className={styles.statTitle}>
              ...
              <School className={styles.statIcon} style={{ color: '#4facfe' }} />
            </Box>
          </CardContent>
        </Card>
      </Grid>
      ...
      <Grid item xs={12} sm={6} md={3}>
        <Card className={styles.statCard} style={{ borderLeft: '4px solid #43e97b' }}>
          <CardContent className={styles.statContent}>
            <Box display="flex" alignItems="center" justifyContent="space-between">
              <Box>
                <Typography variant="h6" className={styles.statTitle}>
              ...
              <Assignment className={styles.statIcon} style={{ color: '#43e97b' }} />
            </Box>
          </CardContent>
        </Card>
      </Grid>
      ...
      case 'bar':
          ...
          return (
            <ResponsiveContainer width="100%" height={400}>
              <ComposedChart data={data}>
                <defs>
                  <linearGradient id="tuitionGradient" x1="0" y1="0" x2="0" y2="1">
                ...
                />
                <Legend />
                <Bar dataKey="amount" name="Doanh thu thực tế" fill="url(#tuitionGradient)" radius={[4, 4, 0, 0]} />
                <Line type="monotone" dataKey="target" name="Mục tiêu" stroke="#ff6b6b" strokeWidth={3} />
              </ComposedChart>
            </ResponsiveContainer>
          );
        }
        return (
          <ResponsiveContainer width="100%" height={400}>
            <BarChart data={data}>
              <defs>
                <linearGradient id={`${selectedReport}Gradient`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={colors[0]} stopOpacity={0.8}/>
                  <stop offset="95%" stopColor={colors[1]} stopOpacity={0.6}/>
                </linearGradient>
              </defs>
              ...
              <Legend />
              <Bar 
                dataKey={selectedReport === 'students' ? 'count' : selectedReport === 'attendance' ? 'rate' : 'amount'} 
                name={selectedReport === 'students' ? 'Số học viên' : selectedReport === 'attendance' ? 'Tỷ lệ điểm danh' : 'Lương cơ bản'} 
                fill={`url(#${selectedReport}Gradient)`} 
                radius={[4, 4, 0, 0]} 
              />
              {selectedReport === 'salary' && (
                <Bar dataKey="bonus" name="Thưởng" fill="#ffd93d" radius={[4, 4, 0, 0]} />
              )}
            </BarChart>
          </ResponsiveContainer>
        );
      ...
      case 'line':
        return (
          <ResponsiveContainer width="100%" height={400}>
            <LineChart data={data}>
              <defs>
                <linearGradient id={`${selectedReport}Area`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={colors[0]} stopOpacity={0.8}/>
      ...
      case 'area':
        return (
          <ResponsiveContainer width="100%" height={400}>
            <AreaChart data={data}>
              <defs>
                <linearGradient id={`${selectedReport}AreaGrad`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={colors[0]} stopOpacity={0.8}/>
                  <stop offset="95%" stopColor={colors[1]} stopOpacity={0.1}/>
                </linearGradient>
              </defs>
            ...
            <Box display="flex" alignItems="center" gap={2} flexWrap="wrap">
              ...
              <Box className={styles.reportButtons}>
                <Button
                  ...
                  className={selectedReport === 'tuition' ? styles.activeButton : ''}
                ...
                <Button
                  ...
                  className={selectedReport === 'students' ? styles.activeButton : ''}
                  startIcon={<PeopleAlt />}
                >
                  Học Viên
                </Button>
                <Button
                  ...
                  className={selectedReport === 'attendance' ? styles.activeButton : ''}
                  startIcon={<School />}
                >
                  Điểm Danh
                </Button>
                <Button
                  ...
                  className={selectedReport === 'salary' ? styles.activeButton : ''}
          ...
          <TableContainer component={Paper} className={styles.tableContainer}>
            ...
            <Table>
              <TableHead className={styles.tableHead}>
                <TableRow>
                  <TableCell className={styles.tableHeaderCell}>Tháng</TableCell>
                  <TableCell align="right" className={styles.tableHeaderCell}>Giá trị</TableCell>
                  <TableCell align="right" className={styles.tableHeaderCell}>Thay đổi</TableCell>
                  <TableCell align="right" className={styles.tableHeaderCell}>Tỷ lệ tăng trưởng</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {tuitionData.map((row, index) => (
                  <TableRow key={row.month} className={styles.tableRow}>
                      ...
                      {index > 0 ? (
                        <Box display="flex" alignItems="center" justifyContent="flex-end" gap={1}>
                          {row.amount > tuitionData[index - 1].amount ? (
                            <TrendingUp fontSize="small" style={{ color: '#2e7d32' }} />
                          ) : (
                            <TrendingDown fontSize="small" style={{ color: '#d32f2f' }} />
                          )}
                          {formatCurrency(Math.abs(row.amount - tuitionData[index - 1].amount))}
                        </Box>
                      ) : (
                        '-'
                      )}
                    ...
                    <TableCell align="right" className={styles.tableCell}>
                      {index > 0 ? (
                        <Chip
                          label={`${(((row.amount - tuitionData[index - 1].amount) / tuitionData[index - 1].amount) * 100).toFixed(2)}%`}
                          size="small"
                          style={{
                            backgroundColor: row.amount > tuitionData[index - 1].amount ? '#e8f5e8' : '#ffebee',
                            color: row.amount > tuitionData[index - 1].amount ? '#2e7d32' : '#d32f2f'
                          }}
                        />
                      ) : (
                        '-'
